...
export const dynamoTableSecondaryIndex = dynamoTable.globalSecondaryIndexes.apply(indexes => (indexes && indexes[0])?.name ?? "gs1")
...
const deployLambda = () => {
  ...
  const lambdaFunc = new aws.lambda.Function("serverlessFullstack-api-lambdaFunc", {
    ...
    environment: {
      variables: {
        db: dynamoTable.name,
        dbIndex1: dynamoTableSecondaryIndex,
  ...
  const lambdaEnvironment = lambdaFunc.environment.apply(env => env?.variables)
  ...
  const apiEndpoint = pulumi.interpolate`${apiGateway.apiEndpoint}/`
    .apply(apiEndpoint => {
...
export const bucketName = frontEndResult?.bucketName
